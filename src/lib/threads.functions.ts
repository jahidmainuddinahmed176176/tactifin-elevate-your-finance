...
// Public chat functions (no auth required for floating chat button)
import { allowPublicChat } from "@/integrations/supabase/public-middleware";

export const createPublicThread = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    const { data: row, error } = await context.supabase
      .from("ai_threads")
      .insert({ 
        user_id: null, // Public thread with no user
        title: data.title ?? "Public chat" 
      })
      .select("id,title,updated_at")
      .single();
    if (error) throw new Error(error.message);

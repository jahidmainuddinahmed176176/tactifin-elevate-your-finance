...
function ChatLayout() {
  ...
  return (
    <div className="grid h-[calc(100vh-8rem)] grid-cols-1 gap-4 md:grid-cols-[260px_1fr]">
          ...
          {threads.map((t) => {
            ...
            return (
              <div key={t.id} className={cn("group flex items-center gap-2 rounded-md px-2 py-1.5 text-sm", active ? "bg-accent" : "hover:bg-accent/50")}>
                <Link to="/chat/$threadId" params={{ threadId: t.id }} className="flex flex-1 items-center gap-2 truncate text-foreground">
